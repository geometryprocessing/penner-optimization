...
def interpolate_metric(
    ...
    @param[in] string output_dir: directory to save interpolated coordinates
...
def add_interpolate_arguments(parser):
    ...
    parser.add_argument("--start_lambdas_path",
                        help="path for start lambda values")
    parser.add_argument("--end_lambdas_path",
                        help="path for end lambda values")
    parser.add_argument("-o", "--output_dir",
                        help="directory for output lambdas and logs")
    parser.add_argument("--num_steps",
                        help="number of interpolation steps", type=int, default=5)


def interpolate_one(args, fname):
    # Get mesh, lambdas, and parameters
    m, _, _, _, C, _, _, _ = script_util.generate_mesh(args, fname)
