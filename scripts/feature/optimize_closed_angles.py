...
def run_one(args, fname):
    ...
    # compute rotation form from frame field (fixing inconsistent matchings for boundary)
    field_generator = penner.IntrinsicNRosyField()
    face_map = np.arange(len(F)) 
    field_generator.initialize(cone_metric)
    field_generator.set_field(cone_metric, vtx_reindex, F, face_map, theta, kappa, period_jump)
    field_generator.fix_inconsistent_matchings(cone_metric)
    rotation_form = field_generator.compute_rotation_form(cone_metric)

    has_bd = False
    Th_hat = np.array(penner.generate_cones_from_rotation_form(cone_metric, vtx_reindex, rotation_form, has_bd))
