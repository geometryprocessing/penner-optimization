# Script to refine a connectivity from an overlay mesh with uv layout.
...
def refine_one(args, fname):
    ...
    try:
        uv_dir = args['uv_dir']
        uv_path = os.path.join(uv_dir, m + "_output", name +  "_overlay.obj")
    ...
    endpoints_path = os.path.join(output_dir, name + '_refined_endpoints')
