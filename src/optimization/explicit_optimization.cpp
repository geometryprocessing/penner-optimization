...
namespace CurvatureMetric
{
      ...
      std::shared_ptr<ProjectionParameters> proj_params)
  {
    ...
    std::unique_ptr<DifferentiableConeMetric> cone_metric = m.set_metric_coordinates(domain_metric_coords);
    SPDLOG_TRACE("Domain metric in range [{}, {}]", domain_metric_coords.minCoeff(), domain_metric_coords.maxCoeff());

    // Project the domain metric to the constraint
    VectorX u = compute_constraint_scale_factors(*cone_metric, proj_params);
    return cone_metric->scale_conformally(u);
  }
      ...
      VectorX &gradient)
  {
    ...
    bool success = constraint_with_jacobian(*cone_metric,
                                            constraint,
                                            constraint_penner_jacobian,
                                            need_jacobian,
                                            only_free_vertices);
      ...
      std::shared_ptr<ProjectionParameters> proj_params)
  {
    ...
    // is satisfied
    while (
        (!success) || (energy > initial_energy * (1 + 1e-8)) || (step_gradient.dot(descent_direction) > 0))
    {
      if (beta < 1e-16)
      {
      ...
      std::shared_ptr<OptimizationParameters> opt_params)
  {
    ...
    compute_optimization_domain(
        ...
        constraint_codomain_matrix,
        domain_coords);
    spdlog::get("optimize_metric")->info("Optimizing {} coordinates with codomain of dimension {}", constraint_domain_matrix.cols(), constraint_codomain_matrix.cols());
    ...
    std::unique_ptr<DifferentiableConeMetric> optimized_cone_metric = compute_domain_coordinate_metric(
        m,
        constraint_domain_matrix,
        domain_coords,
        proj_params);
