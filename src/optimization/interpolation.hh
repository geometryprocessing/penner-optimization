...
public:
  ...
  InterpolationMesh(const Mesh<Scalar> &mesh, const VectorX& scale_factors, bool is_hyperbolic = true);
  ...
  void convert_to_delaunay_hyperbolic_surface(
  ...
  ///
  /// @param[out] flip_sequence: sequence of flips used to make the mesh Delaunay
  void convert_to_delaunay_euclidean_surface(
	  std::vector<int> &flip_sequence
  );
  ...
  void
  follow_flip_sequence(const std::vector<int> &flip_sequence);
  ...
  ///
  /// @return mesh
  Mesh<Scalar> & get_mesh()
  {
    return m_overlay_mesh.cmesh();
  }

	/// Get the overlay mesh of the surface.
  ///
  /// @return overlay mesh
  OverlayMesh<Scalar> const & get_overlay_mesh() const
  {
    return m_overlay_mesh;
  }
