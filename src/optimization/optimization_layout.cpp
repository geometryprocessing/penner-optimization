...
compute_layout_faces(
  ...
  for (int i = 0; i < num_faces; i++)
  {
    ...
    for (size_t j = 0; j < hf.size() - 2; j++)
    {
      ...
      F_out.push_back(std::vector<int>{which_to_group[m.to[h0]], which_to_group[m.to[hf[j + 1]]], which_to_group[m.to[hf[j + 2]]]});
    }
  }
...
compute_layout_components(Mesh<Scalar> &m, const std::vector<Scalar> &u, std::vector<bool>& is_cut_h, int start_h = -1)
{
...
pullback_cut_to_overlay(
  ...
  is_cut_o = std::vector<bool>(m_o.n_halfedges(), false);
