...
TODO: Refactor this energy landscape code and expose to python
      ...
      VectorX &gradient_signs)
  {
    ...
    for (int i = 0; i < num_nodes; ++i)
    {
      // Compute unprojected step
      Scalar step_size = step_sizes[i];
      VectorX reduced_line_step_metric_coords = reduced_metric_coords + step_size *
projected_descent_direction;
      ...
      convergence_ratios[i] = compute_convergence_ratio(step_descent_direction,
step_projected_descent_direction); gradient_signs[i] =
-step_projected_descent_direction.dot(projected_descent_direction);
    }
  }
  */
