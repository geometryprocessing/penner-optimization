...
void init_feature_pybind(pybind11::module& m)
{
    ...
    m.def("write_feature_edges", &write_feature_edges, default_call_guard);
    m.def("generate_refined_feature_field", &generate_refined_feature_field, default_call_guard);

    m.def("parametrize_aligned", &parametrize_aligned, default_call_guard);
    m.def("generate_feature_aligned_frame_field", &generate_feature_aligned_frame_field, default_call_guard);
    m.def("generate_feature_aligned_parameterization", &generate_feature_aligned_parameterization, default_call_guard);
}

void init_additional_feature_pybind(pybind11::module& m)
{
    spdlog::set_level(spdlog::level::info);
    pybind11::call_guard<pybind11::scoped_ostream_redirect, pybind11::scoped_estream_redirect>
        default_call_guard;
