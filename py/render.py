        ...
        for j in range(W):
            if fid_mat[i][j] > -1:
                ...
                if  u_thick < ((u_pt - u_min)%u_unit) <= (u_unit - u_thick) and  v_thick < ((v_pt - v_min)%v_unit) <= (v_unit-v_thick):
                    color_rgb_gd[i,j,:] = np.array(colormap(norm(r_pt))[:4])
                # Shade grid lines darker
                else:
                    color_rgb_gd[i,j,:] = 0.55 * np.array(colormap(norm(r_pt))[:4])
                    color_rgb_gd[i,j,3] = 1

            # Color additional features
            elif fid_mat[i][j] == -1:
                color_rgb_gd[i,j,:] = np.array([1.0,1.0,1.0,0])
            elif fid_mat[i][j] == -2: # Red sphere
                color_rgb_gd[i,j,:] = np.array([255.0, 50.0, 50.0, 255.0]) / 256.0
                #color_rgb_gd[i,j,:] = np.array([223.0, 119.0, 1.0, 255.0]) / 256.0
                #color_rgb_gd[i,j,:] = np.array([0.7,0.1,0.2])
                #color_rgb_gd[i,j,:] = np.array([1.0,0.0, 0.75])
            elif fid_mat[i][j] == -3: # Blue sphere
                color_rgb_gd[i,j,:] = np.array([50.0, 50.0, 255.0, 255.0]) / 256.0
                #color_rgb_gd[i,j,:] = np.array([0.0, 96.0, 128.0, 255.0]) / 256.0
                #color_rgb_gd[i,j,:] = np.array([0.5,0.7,0.35])
                #color_rgb_gd[i,j,:] = np.array([0.0,0.75,1.0])
            elif fid_mat[i][j] == -4:
                color_rgb_gd[i,j,:] = np.array([0,0,0,1])
            elif fid_mat[i][j] == -5:
                color_rgb_gd[i,j,:] = np.array([223.0, 119.0, 1.0, 255.0]) / 256.0
                #color_rgb_gd[i,j,:] = np.array([1,0.1,0.1,1])
